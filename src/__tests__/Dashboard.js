...
describe('Given I am connected as an Admin', () => {
      ...
      describe('Filtered bills', () => {
        test('Filtered by pending status should return 1 bill', () => {
          // Test cases for different bill statuses
        });
      });

      describe('Dashboard UI', () => {
        test('Loading page should be rendered when loading', () => {
          // Test loading state
        });
        ...
        describe('When I am on Dashboard page but back-end sends an error message', () => {
        ...
        describe('When I am on Dashboard page and I click on arrow', () => {
          test('Then, tickets list should be unfolding, and cards should appear', async () => {
            ...
            const icon1 = screen.queryByTestId('arrow-icon1');
            if (icon1) {
            ...
            const icon2 = screen.queryByTestId('arrow-icon2');
            icon2.setAttribute('data-testid', 'arrow-icon2');
            if (icon2) {
            ...
            const icon3 = screen.queryByTestId('arrow-icon3');
            if (icon3) {
        ...
        describe("Given I am a user connected as Admin", () => {
          ...
          describe("When I navigate to Dashboard", () => {
            test('interaction with Dashboard UI elements', async () => {
              // Simulation d'une interaction avec un élément de l'interface utilisateur du tableau de bord
              // Par exemple, cliquer sur l'icône de l'œil
              const handleClickIconEye = jest.fn(Dashboard.handleClickIconEye);
              const eye = screen.getByTestId('icon-eye-d');
              eye.addEventListener('click', handleClickIconEye);
              userEvent.click(eye);
              ...
              // Vérification de l'affichage de la modale après l'interaction
              const modale = screen.getByTestId('modaleFileAdmin');
              expect(modale).toBeTruthy();
            });
            test('fetches bills from mock API GET', async () => {
              // Mock de la fonction de récupération des factures
              mockStore.bills.mockImplementationOnce(() => ({
                list: () => Promise.resolve(bills)
              }));
              window.onNavigate(ROUTES_PATH.Dashboard);
              await waitFor(() => screen.getByText("Validations"));
              // Vérification de l'affichage des factures récupérées
        ...
        describe("When an error occurs on API", () => {
          beforeEach(() => {
            jest.spyOn(mockStore, "bills").mockRejectedValueOnce(new Error("API Error"));
            window.localStorage.setItem('user', JSON.stringify({
              type: 'Admin',
              email: "a@a"
            }));
